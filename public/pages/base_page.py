...
class Base_Page(object):
    ...
    def js_to_element(self,css):
        #weizhi2 = self.driver.find_element_by_xpath(
         #   '//*[@id="app"]/div/div[11]/div/div/div[1]/div[2]/img')  # 先定位到一个元素
        #self.driver.execute_script("arguments[0].scrollIntoView();", weizhi2)  # 让其滚动到这个坐标
        t1 = time.time()
        try:
            js_a = self.get_element(css)
            self.driver.execute_script("arguments[0].scrollIntoView();", js_a)  # 让其滚动到这个坐标

            self.my_print(
                "{0} Execute javascript scripts: {1}, Spend {2} seconds".format(success, css, time.time() - t1))
        except Exception:
    ...
    def get__attribute(self, css, attribute):
        """
        获取元素属性值
    ...
    def swipe_to_ele_by_xpath(self,css):
        #滑动页面到某个元素
        ...
        while i < 10:
            try:
                self.driver.find_element_by_xpath(css).click()  # 尝试点击元素
                break
    ...
    def js_click(self, css):
        """
        输入一个css选择器，使用javascript click元素。
